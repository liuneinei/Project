...
Page({
  ...
  AlbumDelTag: function (event) {
    ...
    wx.showActionSheet({
      ...
      success: function (res) {
        if (!res.cancel) {
          if (res.tapIndex == 0) {
            var index = that.data.CertIndex;

            // 删除事件处理
            var certurls = that.data.CertUrls;
            var _cert = certurls[index];

            var _cert_index = _cert.indexOf('?wxcert')
            if (_cert_index > 0) {
              _cert = _cert.substr(0, _cert_index);
            }
            //删除坐标为 index 的一个元素
            certurls.splice(index, 1);
            ...
            api.wxRequest({
              ...
              success: function (res) {
                var objoin = that.data.objoin;
                objoin.certificate = objoin.certificate.replace(_cert + ',', '');
  ...
  formSubmit:function(event){
    ...
    // 姓名
    var name = fromObj.name;
    // 手机号
    var phone = fromObj.phone;
    // 微信号
    var wxname = fromObj.wxname;
    // 简介
    var desc = fromObj.desc;
    // 介绍
    var notice = fromObj.notice;
    // 培训机构
    var institutions = fromObj.institutions;
    // 推荐人
    var referee = fromObj.referee;

    var objoin = that.data.objoin;
    objoin.status = 1; //0未填写 1已提交审核中 2提交审核失败 3已提交审核成功,
        objoin. isedit= true;
        objoin.name = name;
        objoin.phone = phone;
        objoin.wxname = wxname;
